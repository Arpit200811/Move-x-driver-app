  ...
  const mapHtml = `
    ...
    <body>
      ...
      <script>
        ...
        // Heatmap Polygons
        const heatmapData = ${JSON.stringify(polygons)};
        ...
        // Markers (Orders)
        const customMarkers = ${JSON.stringify(markers)};
        ...
        if (${JSON.stringify(driverLocation)}) {
        ...
        if (${JSON.stringify(route)} && ${JSON.stringify(route)}.length > 0) {
          const rawRoute = ${JSON.stringify(route)};
          const normalizedRoute = rawRoute.map(c => {
